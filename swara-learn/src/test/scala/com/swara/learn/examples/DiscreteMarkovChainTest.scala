...
class DiscreteMarkovChainTest extends FunSuite {
  ...
  test("Gradient writer") {
    ...
    val input =
      ImageIO.read(new File("swara-learn/src/test/resources/gradient.jpg"))
    val gradient = (0 until input.getWidth * input.getHeight)
      .map(pixel =>
            new Color(
                input.getRGB(pixel / input.getWidth, pixel % input.getHeight)))
      .toList
    ...
    (0 until 200).foreach(w =>
          (0 until 200).foreach(h => output.setRGB(h, w, result(w).getRGB)))
    ImageIO.write(
        output,
        "jpg",
        new File("swara-learn/src/test/resources/gradient-result.jpg"))
  }
